    ...
    public partial class FormDeleteSessions : Form
    {
        ...
        public void fff()
        {
            ...
            while (reader.Read())
            {
                ...
                data[data.Count - 1][3] = reader[4].ToString().Substring(0,4);
                data[data.Count - 1][4] = reader[5].ToString();
                data[data.Count - 1][5] = reader[6].ToString();
                data[data.Count - 1][6] = reader[0].ToString();
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command1 = new MySqlCommand("SELECT * FROM `sessions` WHERE `name` = @n", db.getConnection());
            if (textBoxName.Text != "")
        ...
        {
            int znachenie = Convert.ToInt32(dataGridView1[6, e.RowIndex].Value);
            DB db = new DB();
            db.openConnection();
            DataTable table = new DataTable();
            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command = new MySqlCommand("DELETE FROM `sessions` WHERE `id` = @id", db.getConnection());
